...
|---|---|
| `OPENAI_BASE_URL` | Base URL of the OpenAI-compatible API — `https://api.openai.com/v1/`, or `http://192.168.1.102:1234/v1/` for LM Studio or Ollama. A trailing slash is added if missing: it is what maps `/api/models` onto `/v1/models`. Unset, the container stops with an explicit message. |
| `OPENAI_API_KEY` | The key, without the `Bearer ` prefix. Left empty, no `Authorization` header is sent at all, which is what a local backend that does not authenticate expects. A double quote is refused, it would break the NGinX configuration. |
| `OPENAI_MODEL_ALIASES` | Display names for the models, as a comma-separated list of `id=alias` pairs — `Qwen3.6-35b-a3b=Fast, gpt-5=Smart`. Only what is shown changes — the menu, and the line above each answer: the identifier is what the completion request carries, so an alias may be anything, and a model left out keeps its identifier. A single quote is refused, it would break the NGinX configuration. A malformed pair is reported in the container logs, and an alias naming a model the backend does not offer in the browser console; neither stops anything. |
| `OPENAI_MODEL` | Model a first visit starts on, named by identifier or by alias, case-insensitively — `Fast` as readily as `Qwen3.6-35b-a3b`, `qwen3.6-35b-a3b` as readily as `Qwen3.6-35B-A3B-bf16`. A choice made in the interface is remembered and outranks it; unset, or naming a model the menu does not hold, the page falls back to the first chat model of the list and says so in the browser console. |
| `OPENAI_MODEL_IGNORE` | Models to leave out of the menu, as a comma-separated list of identifiers where `*` stands for any run of characters — `whisper-*, tts-*, dall-e-*, *embedding*` drops four families the API offers and no chat interface can use. Matching is on the identifier, never on the alias, and case is disregarded. A pattern matching nothing is reported in the browser console; a list emptying the whole menu is reported on the page, since nothing would be left to talk to. |
| `OPENAI_MODEL_EXPOSE` | The inverse, as the same comma-separated patterns — the menu keeps only the models that match, `gpt-5, qwen3.6-35b-a3b` for a single-purpose installation. Unset, everything is kept. Set together, the expose list narrows the catalogue down and the ignore list takes a few of those back out. Matching is on the identifier, never on the alias, and case is disregarded. A pattern matching nothing is reported in the browser console; a list matching nothing at all is reported on the page, since nothing would be left to talk to. |
...
**Model** lists what the backend offers, narrowed by `OPENAI_MODEL_EXPOSE` when one is configured and less what `OPENAI_MODEL_IGNORE` takes out, and is filled on load. The choice is remembered across reloads, unlike the three settings that follow, which start afresh every time; a first visit starts on `OPENAI_MODEL`. `OPENAI_MODEL_ALIASES` renames the entries, for a menu reading `Fast` and `Smart` rather than a column of identifiers; the identifier stays a hover away, on the menu and on the answer alike.

**Thinking** on `Off` asks the model not to reason: a faster and cheaper answer, when the question can do without. A model that accepts a reasoning *effort* shows its levels in the menu between the two — probed on load, and again on every model switch, with a refused value whose error names the accepted ones, then verified with a one-token request each, so only the levels the backend really answers are offered. The discovery runs once per model per session, is per model, and never blocks the page — a probe that caught the backend down leaves nothing cached and tries again on the next model switch. On `Auto` the model decides, and its reasoning is shown in a panel above the answer, folded away once the answer proper begins. A backend that refuses the request is retried without the parameters, and the answer says so.
